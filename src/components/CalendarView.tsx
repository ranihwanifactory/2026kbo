...
export default function CalendarView({ schedule, getTeamLogo, filterTeam }: CalendarViewProps) {
  ...
  return (
    <div className="glass-card rounded-3xl overflow-hidden bg-white border border-slate-200 shadow-xl">
      <div className="bg-slate-50 p-4 md:p-6 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
        <h3 className="text-lg md:text-xl font-display font-bold text-slate-800">2026년 3월</h3>
        <div className="flex gap-3 md:gap-4 text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-widest">
      ...
      <div className="grid grid-cols-7 border-b border-slate-100">
        {weekdays.map((day, i) => (
          <div key={day} className={`py-2 md:py-4 text-center text-[10px] md:text-xs font-black uppercase tracking-widest ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-slate-400'}`}>
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7">
        ...
        {calendarDays.map(day => {
          ...
          return (
            <div 
              key={day} 
              className={`aspect-square border-r border-b border-slate-100 p-0.5 md:p-2 flex flex-col relative group transition-colors ${isGameDay ? 'bg-white' : 'bg-slate-50/30'}`}
            >
              <span className={`text-[10px] md:text-sm font-bold mb-0.5 md:mb-1 ${(startDay + day - 1) % 7 === 0 ? 'text-red-500' : (startDay + day - 1) % 7 === 6 ? 'text-blue-500' : 'text-slate-400'}`}>
                {day}
              </span>
              ...
              <div className="flex-1 overflow-y-auto no-scrollbar space-y-0.5">
                {dayGames?.games.map((game, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-0.5 scale-90 md:scale-100">
                      <span>{game.away}</span>
                ...
                {isRestDay && (
                  <div className="flex-1 flex items-center justify-center">
                    <span className="text-[6px] md:text-[10px] font-bold text-slate-300 uppercase tracking-tighter">REST</span>
                  </div>
                )}
              </div>
            </div>
